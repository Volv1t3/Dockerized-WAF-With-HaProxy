<!--
	...
	Displays product grid with search, filtering, and pagination.
	Shows product cards with images, names, prices, and quick actions.
-->

<script>
	/**
	 * @fileoverview Storefront page component.
	 * @module routes/store/+page
	 * @description This component serves as the main storefront, displaying a paginated grid of agricultural products. It includes search functionality, handles loading states, errors, and an empty state. Users can browse products and view details.
	...
	function handleSearch() {
		...
		goto(`/store?${params.toString()}`, { replaceState: true });
	}
	 ...
	 * @description Clears the current search query and navigates back to the base store page.
	 * @returns {void}
	 *
	 * Steps:
	 *   1. Clear the `inputValue`.
	 *   2. Navigate to the `/store` route, replacing the current history state.
	 */
	function clearSearch() {
		// Step 1: Clear the search input value.
		inputValue = '';
		// Step 2: Navigate to the base store page.
		goto('/store', { replaceState: true });
	}
	
	// Life-cycle method: onMount
	// Step 1: Initialize search query, input value, and current page from URL parameters when the component is first mounted.
	// Step 2: Load products based on initial URL parameters with timeout to show animation.
	onMount(() => {
	...
	// Life-cycle method: afterNavigate
	// Step 1: This hook runs after every navigation.
	// Step 2: Extract new query and page parameters from the URL.
	// Step 3: Check if the current route is '/store' and if the query or page has changed.
	// Step 4: If changes are detected, update component state and reload products.
	afterNavigate(() => {
		const urlSearchParams = $page.url.searchParams;
		const newQuery = urlSearchParams.get('q') || '';
		const newPage = parseInt(urlSearchParams.get('page') || '1', 10);
		
		// Only reload if we're actually on the store page and parameters have changed.
		if ($page.route.id === '/store' && (newPage !== currentPage || newQuery !== searchQuery)) {
	...
	// Reactive statement: React to URL changes (redundant with afterNavigate, but kept for demonstration)
...
<svelte:head>
	<title>Store - AgriCommerce</title>
	<meta name="description" content="Shop quality agricultural products from local farmers. Fresh produce, organic vegetables, and sustainable farming supplies." />
</svelte:head>
...
<div class="store-container container">
	<div class="store-header">
		<h1>Our Store</h1>
		<p>Quality agricultural products from local farmers</p>
	</div>
	...
	<div class="store-content">
		...
		{:else if products.length === 0}
			...
			<div class="empty-state card">
				...
				{#if searchQuery}
					<button class="btn btn-primary" on:click={clearSearch}>
						View All Products
					</button>
				{/if}
			</div>
				...
				{#each products as product (product.id)}
					<div class="product-card card">
						...
						<div class="product-actions">
							...
							<a href="/products/{product.slug}" class="btn btn-outline btn-small btn-full">
								View Details
							</a>
						</div>
					</div>
						...
						{#each Array.from({length: Math.min(5, totalPages)}, (_, i) => {
							...
							{#if pageNum <= totalPages}
								<button
									...
									class:btn-outline={pageNum !== currentPage}
